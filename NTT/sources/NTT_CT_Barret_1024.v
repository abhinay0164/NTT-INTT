...
module NTT_CT_Barret_1024 #(N=1024, Q=12289, W=10322, BARRETT_M=21843 ,WIDTH=14, STAGES=10, BUTTERFLIES=N/2)(
    input clk, rst, start,
    ...
    //============= COMPUTE STATE COMBINATIONAL LOGIC USING BARRETT REDUCTION =============
    wire [WIDTH-1:0] U = dout1; //U
    wire [2*WIDTH-1:0] mult_out = dout2 * dout_tw; //double bits as we are multiplying
    
    // Start of BARRETT reduction
    wire [2*WIDTH + $clog2(BARRETT_M) - 1:0] barrett_mult = mult_out * BARRETT_M; //28 bit x 15 bit storage 
    wire [WIDTH - 1 :0] estimate = barrett_mult[2*WIDTH + $clog2(BARRETT_M) -1 : 2*WIDTH];
    wire [$clog2(BARRETT_M) - 1 :0] difference = mult_out[$clog2(BARRETT_M) - 1 : 0] - (estimate * Q); // 2(WIDTH) is required because we are multiplying again
    wire [WIDTH - 1 : 0] V = (difference >= Q)? (difference - Q) : difference;
    // End of BARRETT reduction
